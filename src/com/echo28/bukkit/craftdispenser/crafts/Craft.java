...
abstract public class Craft
{
	...
	protected boolean checkVerticalItems(ItemSpec[] items)
	{
		...
		if (items.length == 2)
		{
			for (int i = 0; i < 6; i++)
			{
				if (items[0].matchesStack(inventory.getItem(i)) && items[1].matchesStack(inventory.getItem(i + 3)))
				{
		...
		else if (items.length == 3)
		{
			for (int i = 0; i < 3; i++)
			{
				if (items[0].matchesStack(inventory.getItem(i)) && items[1].matchesStack(inventory.getItem(i + 3)) && items[2].matchesStack(inventory.getItem(i + 6)))
				{
					subtractItem(i, items[0].amount);
					subtractItem(i + 3, items[1].amount);
